...
\[
\boxed{
\text{No declared observer/readout, no declared visibility.}
}
\]
...
\[
\boxed{
\text{If relation is primitive, then declared relation requires declared observer/readout.}
}
\]
...
\[
\boxed{
\text{a relation cannot be declared for inference unless its readout or visibility role is declared.}
}
\]
...
\[
\boxed{
\text{boundary without readout is not inspectable;}
}
\]

\[
\boxed{
\text{readout without boundary has no declared distinction to read;}
}
\]

\[
\boxed{
\text{mediation without both is an underdeclared relation.}
}
\]

Thus No Undeclared Observer is not a claim that observation alone constitutes relation. It is the \(\mathsf O\)-face of declared relation, co-required with \(\mathsf Z\) and \(\mathsf R\).
...
\[
\boxed{
\text{declared visibility/readout role internal to the relation context}.
}
\]
...
- \(\operatorname{Adm}\), the admission rule;
- \(\operatorname{Update}\), the trace update;
- target visibility through \(D_Y'\).
...
\mathfrak O
=
(\mathcal C,\mathcal E,\operatorname{Read},\operatorname{Adm},\operatorname{Update})
\]
...
3. \(\operatorname{Read}\) is the readout map;
4. \(\operatorname{Adm}\) is the admission rule;
5. \(\operatorname{Update}\) is the trace update, if recursive learning or calibration is involved.
...
## 6.1 Observer/readout alone: \(\{\mathsf O\}\)
...
No Undeclared Observer is the \(\mathsf O\)-face of No Undeclared Relation.

---

## 7.1 Observer/readout substitute

An **observer/readout substitute** is any structure used to perform the \(\mathsf O\)-role in a local triad:
...
This is the Triadic Substitution Principle applied to the \(\mathsf O\)-role. If the observer/readout substitute does nontrivial relation work, it must have declared mediation, visibility, and boundary conditions sufficient for that work. Otherwise the observer/readout substitute is underdeclared. \(\square\)
...
| no calibrator, no approximation | target/tolerance/evaluator/admission/update readout | Theorem 8.3 |
...
Observer/readout is the \(\mathsf O\)-face of declared relation.

In the triadic recursion decomposition, observer/readout corresponds to substitution recursion:

\[
\boxed{
\mathsf O\text{-recursion}=\text{substitution / role-visibility recursion}.
}
\]
...
\mathsf Z
\Rightarrow
(\mathsf Z_Z,\mathsf R_Z,\mathsf O_Z).
\]
...
\text{role visibility}
\to
\text{substitution pressure}
\to
\text{triadic declaration}
\to
\text{admitted recursion}.
}
\]
...
\[
\boxed{
\text{undeclared observer/readout}
\Rightarrow
...
Both are required faces of declared relation.
