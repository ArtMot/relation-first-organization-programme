...
\[
\boxed{
\text{No declared boundary, no declared distinction.}
}
\]
...
The paper proves that every distinction claim, exactness claim, approximation claim, support claim, failure claim, local/global claim, and transfer claim requires a declared boundary. Without such a boundary, the claim is not merely imprecise; it is an underdeclared relation.

The central theorem is the **Boundary-Face Necessity Theorem**:

\[
\boxed{
\text{If relation is primitive, then declared relation requires declared boundary.}
}
\]
...
\[
\boxed{
\text{a relation cannot be declared without declaring the boundary that makes its distinctions determinate.}
}
\]
...
\[
\boxed{
\text{what is being distinguished so that relation can be asserted?}
}
\]
...
- \(D_Y'\) declares the target boundary;
- \(\varepsilon\) declares tolerance;
- \(\operatorname{Adm}\) declares success/failure boundary;
...
\mathfrak B_\partial
=
(D,\partial D,\operatorname{Adm},\varepsilon,\operatorname{Fail})
\]

where:

1. \(D\) is the distinction family;
2. \(\partial D\) is the boundary or support condition;
3. \(\operatorname{Adm}\) is the admissibility rule;
4. \(\varepsilon\) is tolerance, if approximation is involved;
5. \(\operatorname{Fail}\) records boundary failure modes.
...
## 6.3 Observer/readout alone: \(\{\mathsf O\}\)
...
\[
R_{\rm declared}\Rightarrow(\mathsf R,\mathsf O,\mathsf Z).
\]

No Undeclared Boundary is the \(\mathsf Z\)-face of No Undeclared Relation.

---

## 7.1 Boundary substitute

A **boundary substitute** is any structure used to perform the \(\mathsf Z\)-role in a local triad:
...
This is the Triadic Substitution Principle applied to the \(\mathsf Z\)-role. If the boundary substitute does nontrivial relation work, it must have declared mediation, visibility, and boundary conditions sufficient for that work. Otherwise the boundary substitute is underdeclared. \(\square\)

---

## 7.3 Boundary coupling

When a boundary from one triad is used in another triad, it forms a role interface.
...
and the recovered distinctions match \(D_Y'\). Therefore the calibration error is zero relative to that local target. This is terminal contact for the local task boundary. It does not imply global completion unless the target itself is global and total. \(\square\)
...
\[
\boxed{
\text{global exactness with declared asymptotic/global boundary}
\neq
\text{undeclared totalization}.
}
\]
...
---
# 9. Boundary no-go family

The following are summary projections of the theorems above.

| No-go | Boundary being demanded | Source theorem |
|---|---|---|
| no undeclared threshold | admissible / inadmissible cut | Theorem 8.1 |
| no undeclared support | support / exterior boundary | Theorem 8.1 |
| no undeclared exactness | local target boundary | Theorem 8.3 |
| no undeclared approximation | tolerance boundary | Theorem 8.2 |
| no undeclared failure | failure/admission boundary | Theorem 8.1 |
| no undeclared globality | global or asymptotic target boundary | Theorem 8.4 + Remark 8.5 |
| no terminal totalization under proper transfer | full target boundary | NUR no-total-preservation discipline |
...
Boundary is the \(\mathsf Z\)-face of declared relation.

In the triadic recursion decomposition, boundary corresponds to residual recursion:

\[
\boxed{
\mathsf Z\text{-recursion}=\text{residual recursion}.
}
\]
...
\to
\text{recursion pressure}
\to
\text{admitted update}
\to
\text{realized recursion}.
}
\]
...
is meaningful only because \(\ell\) and \(u\) declare boundary.
...
\[
\boxed{
\text{unreachable boundary can be declared;}
}
\]

\[
\boxed{
\text{undeclared globality cannot.}
}
\]
...
\[
\boxed{
\text{undeclared boundary}
\Rightarrow
\text{underdeclared relation}
\Rightarrow
\text{NUR failure}.
}
\]
...
Polchinski, J. (1992). *Effective Field Theory and the Fermi Surface*. arXiv:hep-th/9210046.
